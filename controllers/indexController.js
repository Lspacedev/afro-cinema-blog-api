...
const validateSignUp = [
    body("username").trim()
      ...
      .isLength({ min: 1, max: 25 }).withMessage(`Username ${lengthErr}`),
    body("email").isEmail().withMessage('Not a valid e-mail address'),
    body("password").isLength({ min: 5, max: 25 }).withMessage(`Password ${passLengthErr}`),
...
const postSignUpForm = [validateSignUp, async(req, res, next) => {
